    ...
    V: Into<PathBuf>,
{
    ...
    if semantic_version.is_none() && abi_version > ABI_VERSION_MIN {
        warn!("Warning: No `tree-sitter.json` file found in your grammar, this file is required to generate with ABI {abi_version}. Using ABI version {ABI_VERSION_MIN} instead.
